...
use tri_mesh::{Mesh, VertexID, Vec3};
...
impl CurvatureAnalyzer {
    ...
    pub fn calculate_gaussian_curvature(&self) -> Vec<f64> {
        ...
        let mut area_sum: HashMap<VertexID, f64> = HashMap::with_capacity(self.mesh.no_vertices());
        let mut angle_sum: HashMap<VertexID, f64> = HashMap::with_capacity(self.mesh.no_vertices());

        for face_id in self.mesh.face_iter() {
            let face = self.mesh.face_positions(face_id);
            let angles = self.calculate_angles_for_face(face); // This returns [f64; 3]
            let (v1, v2, v3) = self.mesh.face_vertices(face_id);
            let area = self.mesh.face_area(face_id) / 3.0; // Assuming Voronoi area

            for (vertex_id, angle) in [(v1, angles[0]), (v2, angles[1]), (v3, angles[2])].iter() {
                *area_sum.entry(*vertex_id).or_insert(0.0) += area;
                *angle_sum.entry(*vertex_id).or_insert(0.0) += angle;
            }
        }

        for vertex_id in self.mesh.vertex_iter() {
            let area = area_sum.get(&vertex_id).cloned().unwrap_or(0.0);
            let angle = angle_sum.get(&vertex_id).cloned().unwrap_or(0.0);
            let index_as_u32: u32 = *vertex_id;
            let index_as_usize: usize = index_as_u32 as usize;
            curvatures[index_as_usize] = (2.0 * std::f64::consts::PI - angle) / area;
        }
    ...
    fn calculate_angles_for_face(&self, face_positions: (Vec3, Vec3, Vec3)) -> [f64; 3] {
        ...
        let vertices = [
            ...
            Vector3::new(pos3.x, pos3.y, pos3.z),
        ];
