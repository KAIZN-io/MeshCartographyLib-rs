...
impl MeshCutting {
    ...
    fn collect_v_positions(&self, mesh: &Mesh, edge_path: Vec<tri_mesh::HalfEdgeID>) -> Vec<Vector3<f64>> {
    ...
    fn get_cutline(&self, mesh: &Mesh, edge_path: Vec<tri_mesh::HalfEdgeID>) -> Vec<VertexID> {
    ...
    fn collect_zigzag_cutline(&self, mesh: &Mesh, edge_path: Vec<tri_mesh::HalfEdgeID>) -> Vec<tri_mesh::HalfEdgeID> {
        // 0.1 Get the vertices of the cut line
        let cutline_vertices = self.get_cutline(&mesh, edge_path.clone());
        ...
        for i in 0..(edge_path.len() - 1) {
            ...
            while true {
                ...
                let (v1, _) = mesh.edge_vertices(h_center);
                if v1 == vertice_after {
                    break;
                }
    ...
    fn get_affected_faces(&self, mesh: &Mesh, zigzag_cutline: Vec<tri_mesh::HalfEdgeID>) -> Vec<FaceID> {
    ...
    pub fn open_mesh_along_seam(&self, mesh: tri_mesh::Mesh, edge_path: Vec<tri_mesh::HalfEdgeID>) -> Mesh {

        // 0. Get all vertices of the open mesh
        let vertex_coord = self.collect_v_positions(&mesh, edge_path.clone());

        // 1. Find all halfedges that are inside the affected faces
        let zigzag_cutline = self.collect_zigzag_cutline(&mesh, edge_path.clone());

        // 2. Get the affected faces where at least one halfedge from zigzag_cutline is inside
        let affected_faces = self.get_affected_faces(&mesh, zigzag_cutline.clone());
